...
class Client:
    ...
    def get_cookie(
        self, key: str, domain: str = "127.0.0.1", path: str = "/"
    ) -> Cookie | None:
    ...
    def set_cookie(
        ...
        **kwargs: t.Any,
        ...
        .. versionchanged:: 2.3
            The ``domain`` parameter defaults to ``127.0.0.1``.
        """
        if self._cookies is None:
            raise TypeError(
                "Cookies are disabled. Create a client with 'use_cookies=True'."
    ...
    def _add_cookies_to_wsgi(self, environ: WSGIEnvironment) -> None:
        ...
        server_name = url.hostname or "127.0.0.1"
        value = "; ".join(
    ...
    def run_wsgi_app(
        ...
        self._update_cookies_from_response(
            url.hostname or "127.0.0.1", url.path, rv[2].getlist("Set-Cookie")
        )
        return rv
...
def create_environ(*args: t.Any, **kwargs: t.Any) -> WSGIEnvironment:
    ...
    127.0.0.1:80) or a full path to the request with scheme, netloc port and
