<template>
    ...
    <div class="w-full max-w-full px-6">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-4">
          <button @click="goToResult" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-3 rounded-full shadow-lg transition hover:scale-105">Result</button>
          <button @click="goToAnalytics" class="bg-purple-500 hover:bg-purple-600 text-white font-semibold px-6 py-3 rounded-full shadow-lg transition hover:scale-105">Analytics</button>
        </div>
      </div>
      ...
      <div class="rounded-xl shadow-xl overflow-x-auto bg-white w-full">
        <table class="w-full border-separate border-spacing-0 min-w-[980px]">
          ...
          <tbody>
            <tr
              v-for="(attempt, idx) in filteredAttempts"
              :key="attempt.Attempt_Id"
              class="hover:bg-blue-50 border-b border-gray-100 text-sm"
            >
  ...
  <!-- UNAUTHORIZED UI -->
  <div v-else class="min-h-screen flex items-center justify-center bg-red-50">
  ...
  <transition name="fade">
    ...
    >
      <div v-if="activeFilter === 'student'">
        <input v-model="filters.studentId" type="text" placeholder="Search Student ID..."
        ...
        <div class="flex justify-center gap-8 text-2xl font-bold">
          <span @click="toggleStudentSort('asc')" :class="filters.studentSort === 'asc' ? 'text-blue-600 cursor-pointer' : 'text-gray-400 hover:text-blue-600 cursor-pointer'">↑</span>
          <span @click="toggleStudentSort('desc')" :class="filters.studentSort === 'desc' ? 'text-blue-600 cursor-pointer' : 'text-gray-400 hover:text-blue-600 cursor-pointer'">↓</span>
        </div>
      </div>
      ...
      <div v-if="activeFilter === 'marks'">
        ...
        <div class="flex justify-center gap-8 text-2xl font-bold">
          <span @click="toggleMarksSort('asc')" :class="filters.marksSort === 'asc' ? 'text-blue-600 cursor-pointer' : 'text-gray-400 hover:text-blue-600 cursor-pointer'">↑</span>
          <span @click="toggleMarksSort('desc')" :class="filters.marksSort === 'desc' ? 'text-blue-600 cursor-pointer' : 'text-gray-400 hover:text-blue-600 cursor-pointer'">↓</span>
        </div>
      </div>
...
const toggleStudentSort = (direction) => {
  filters.value.studentSort = filters.value.studentSort === direction ? '' : direction
  filters.value.marksSort = ''
}

const toggleMarksSort = (direction) => {
  filters.value.marksSort = filters.value.marksSort === direction ? '' : direction
  filters.value.studentSort = ''
}

const clearCurrentFilter = () => {
  if (activeFilter.value === 'student') { filters.value.studentId = ''; filters.value.studentSort = '' }
  if (activeFilter.value === 'marks')   { filters.value.minMarks = null; filters.value.marksSort = '' }
  if (activeFilter.value === 'status')  { filters.value.status = '' }
}
...
const filteredAttempts = computed(() => {
  ...
  if (filters.value.studentSort === 'asc')  result.sort((a, b) => a.Applicant_Id - b.Applicant_Id)
  if (filters.value.studentSort === 'desc') result.sort((a, b) => b.Applicant_Id - a.Applicant_Id)
...
const fetchAttempts = async () => {
  try {
    ...
    if (!res.data.success) { error.value = 'Unauthorized Access'; return }
...
const viewAnswers  = (attemptId) => router.push({ name: 'ViewAnswers', params: { attemptId } })
const goToResult   = () => router.push(`/exam/${examId.value}/result`)
